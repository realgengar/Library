--[[
 Magnus Primeiro A Bagunça A Lib Redz
  Choros dps
]]
...
function redzlib:Notify(Configs)
	...
	local NotificationStroke = Create("UIStroke", NotificationFrame, {
		Color = Color3.fromRGB(179, 0, 255), -- Cor roxa
		Thickness = 2,
		ApplyStrokeMode = "Border"
	})
	...
	local TitleLabel = Create("TextLabel", NotificationFrame, {
		...
		Position = UDim2.new(0, 44, 0, 4), -- Título
		BackgroundTransparency = 1,
		Text = Title,
		TextColor3 = Color3.fromRGB(255, 255, 255),
		TextSize = 11, -- Tamanho menor
	...
	local DescLabel = Create("TextLabel", NotificationFrame, {
		...
		Position = UDim2.new(0, 44, 0, 18), -- Descrição,
		BackgroundTransparency = 1,
		Text = Description,
...
local function CreateCyberpunkParticle()
	...
	local ParticleData = {
		...
		StartTime = tick(),
		Speed = speed,
		Direction = Vector2.new(0, -speed), -- Somente para cima
...
local function UpdateCyberpunkParticles()
	...
	for i = #ActiveParticles, 1, -1 do
		...
		-- Remoção apenas quando sair da tela ou acabar tempo de vida
...
-- Conexão principal das partículas (SUBSTITUIR a conexão existente)
local ParticleConnection = RunService.Heartbeat:Connect(function()
	UpdateCyberpunkParticles()
	SpawnCyberpunkSystem()
end)

-- Sistema de resposta a interação do usuário (REMOVIDO O BURST EFFECT)
-- Mantém apenas o sistema básico de partículas roxas subindo
