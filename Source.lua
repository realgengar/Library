...
function redzlib:MakeWindow(Configs)
	...
	local function ApplyConfig(data)
		...
		if data.MaxParticles     then ParticleConfig.MaxParticles     = data.MaxParticles     end
		if data.SpawnRate        then ParticleConfig.SpawnRate        = data.SpawnRate        end
		if data.ParticleLifetime then ParticleConfig.ParticleLifetime = data.ParticleLifetime end
		if data.GlowIntensity    then ParticleConfig.GlowIntensity    = data.GlowIntensity    end
		if data.WindDrift        then ParticleConfig.WindDrift        = data.WindDrift        end
		if data.Gravity          then ParticleConfig.Gravity          = data.Gravity          end
		if data.ParticleSize then
			if data.ParticleSize.Min then ParticleConfig.ParticleSize.Min = data.ParticleSize.Min end
			if data.ParticleSize.Max then ParticleConfig.ParticleSize.Max = data.ParticleSize.Max end
		end
		if data.ParticleSpeed then
			if data.ParticleSpeed.Min then ParticleConfig.ParticleSpeed.Min = data.ParticleSpeed.Min end
			if data.ParticleSpeed.Max then ParticleConfig.ParticleSpeed.Max = data.ParticleSpeed.Max end
		end
	end

	local function FetchParticleConfig()
		local ok, result = pcall(function()
			local raw
			-- suporte aos principais executors
			if syn and syn.request then
				raw = syn.request({ Url = PARTICLE_CONFIG_URL, Method = "GET" }).Body
			elseif http and http.request then
				raw = http.request({ Url = PARTICLE_CONFIG_URL, Method = "GET" }).Body
			elseif request then
				raw = request({ Url = PARTICLE_CONFIG_URL, Method = "GET" }).Body
			elseif (game:GetService("HttpService")).GetAsync then
				raw = game:GetService("HttpService"):GetAsync(PARTICLE_CONFIG_URL)
			else
				return
			end
			if not raw or #raw < 5 then return end
			local fn, err = loadstring(raw)
			if not fn then warn("[ParticleConfig] Parse error: " .. tostring(err)) return end
			ApplyConfig(fn())
		end)
		if not ok then
			warn("[ParticleConfig] Fetch error: " .. tostring(result))
		end
	end

	-- Carrega ao iniciar
	task.spawn(FetchParticleConfig)

	-- Recarrega periodicamente enquanto o hub existir
	task.spawn(function()
		while task.wait(RELOAD_INTERVAL) do
	...
	local function CreateParticle()
		...
		local spd  = math.random(ParticleConfig.ParticleSpeed.Min, ParticleConfig.ParticleSpeed.Max)
		local startX = math.random(10, math.max(10, cs.X - 10))
		local startY = cs.Y + 20
		local color  = Theme["Color Theme"]
		...
		Create("UICorner", Particle, { CornerRadius = UDim.new(0.5, 0) })

		-- Glow ao redor da partícula
		local glowSize = size + math.floor(size * 0.6)
		local Glow = Create("Frame", Particle, {
			Size             = UDim2.fromOffset(glowSize, glowSize),
		...
		Create("UICorner", Glow, { CornerRadius = UDim.new(0.5, 0) })

		-- Fade in
		local targetAlpha = math.max(0, math.min(1, ParticleConfig.GlowIntensity))
		CreateTween({ Particle, "BackgroundTransparency", 0.15, 0.6 })
		CreateTween({ Glow,     "BackgroundTransparency", targetAlpha + 0.2, 0.6 })

		-- Drift lateral aleatório baseado no WindDrift
		local drift = (math.random() - 0.5) * 2 * ParticleConfig.WindDrift
	...
	local function UpdateParticles()
		...
		for i = #ActiveParticles, 1, -1 do
			...
			-- Remove se expirou ou saiu da tela pelo topo
			if elapsed >= ParticleConfig.ParticleLifetime
			or p.Frame.Position.Y.Offset < -30 then
				CreateTween({ p.Frame, "BackgroundTransparency", 1, 0.4 })
				CreateTween({ p.Glow,  "BackgroundTransparency", 1, 0.4 })
				local f = p.Frame
				...
				-- Movimento: sobe com velocidade base + gravidade + drift lateral
				local vy = -(p.SpeedY + ParticleConfig.Gravity) * dt
				local vx = p.Drift * dt
				...
				-- Fade out na última 20% da vida
				local lifeRatio = elapsed / ParticleConfig.ParticleLifetime
				if lifeRatio > 0.80 then
					local fade = 1 - ((lifeRatio - 0.80) / 0.20)
					p.Frame.BackgroundTransparency = math.clamp(1 - (0.15 * fade), 0, 1)
					p.Glow.BackgroundTransparency  = math.clamp(1 - (p.BaseAlpha * fade), 0, 1)
				end
			end
		end
	end
	...
	-- Força reload manual da config do GitHub sem esperar o intervalo
	function Window:ReloadParticleConfig()
		task.spawn(FetchParticleConfig)
	end
